...
import {convert as mhtml2html} from "@luminati-io/mhtml2html";

export const HTML_FILE_EXTENSIONS = ["html", "htm", "mhtml"];
export const VIEW_TYPE_HTML = "html-view";
export const ICON_HTML = "doc-html";

export type MainView = HTMLDivElement & {
	app: App;
...
export type ExperimentalHTMLIframeElement = HTMLIFrameElement & {
	// Only available in chrome based browsers
	csp: string;
};

export type CustomHTMLIframeElement = ExperimentalHTMLIframeElement & {
	mainView: MainView;
};
...
export class HtmlView extends FileView {
	...
	constructor(leaf: WorkspaceLeaf, settings: HtmlPluginSettings) {
		super(leaf);
	...
	async onLoadFile(file: TFile): Promise<void> {
		...
		try {
			...
			if (file.extension === "mhtml") {
				const decoder = new TextDecoder();
				htmlStr = decoder.decode(contents); // decode with UTF8
				htmlStr = mhtml2html(htmlStr).window.document.documentElement.outerHTML; // convert MHTML to HTML
			} else
				try {
					// the HTML file made by SingleFileZ
					globalThis.zip = zip;
					const { docContent } = extract(
			...
			this.mainView = this.contentEl.createDiv() as MainView;
			this.mainView.setAttribute(
				"style",
			...
			const searchBar: HTMLDivElement =
				this.mainView.querySelector("#ohpMainView")!;
			const iframe: CustomHTMLIframeElement =
				this.mainView.querySelector("#ohpIframe")!;
			...
			switch (this.settings.opMode) {
				case HtmlPluginOpMode.Balance:
					...
					iframe.csp =
						"script-src 'none'; require-trusted-types-for 'script'; object-src 'none'; frame-src https: http: mediastream: blob:;";
				...
				case HtmlPluginOpMode.HighRestricted:
					...
					purifier.addHook(
						...
						ohpAfterSanitizeAttributes
					); // disable some elements to avoid XSS attacks
					const cleanHtmlHR = purifier.sanitize(
						htmlStr,
						hrModeConfig
					);
					// iframe.sandbox = "allow-forms allow-modals allow-pointer-lock allow-popups allow-presentation allow-top-navigation-by-user-activation";
					iframe.csp =
						"default-src 'none'; script-src 'none'; object-src 'none'; frame-src https: http: mediastream: blob:; font-src 'self' data:; img-src 'self' data:; style-src 'unsafe-inline'; media-src 'self' data:; ";
					iframe.srcdoc = cleanHtmlHR;
					break;

				case HtmlPluginOpMode.Text:
					const cleanHtmlText = window.DOMPurify.sanitize(
						htmlStr,
						textModeConfig
					);
					iframe.setAttribute("sandbox", "allow-same-origin");
					iframe.csp =
						"default-src 'none'; script-src 'none'; object-src 'none'; frame-src 'none'; font-src 'self' data:; img-src 'none'; style-src 'unsafe-inline'; media-src 'none'; ";
			...
			iframe.onload = async function () {
				if (applyAnchorFix) {
					...
					iframe.contentWindow!.addEventListener(
						"click",
						sdFixAnchorClickHandler
					);
				}

				await restoreStateBySettings(
					iframe.contentWindow!.document,
					iframe.mainView.settings
				);
	...
	onPaneMenu(
		...
		if (source !== "more-options")
			// only handle 'more-options' onMoreOptionsMenu()
			return;

		menu.addItem((item) => {
			item.setTitle(i18next.t("interface.menu.find"))
		...
		menu.addItem((item) => {
			item.setTitle(i18next.t("commands.zoom-in"))
		...
		menu.addItem((item) => {
			item.setTitle(i18next.t("commands.zoom-out"))
		...
		menu.addItem((item) => {
			item.setTitle(i18next.t("commands.reset-zoom"))
				.setIcon("reset")
				.onClick(async () => {
...
function sdFixAnchorClickHandler(evt: MouseEvent) {
	const aElm = evt
		.composedPath()
		.find((elm: Element) => elm.nodeName === "A") as HTMLAnchorElement;
	const regex = /href\s*=\s*['"]\s*#/gim; // Regex for checking <a href="#xxx">

	// ignore non-internal link
	if (!aElm || !aElm.href || !regex.test(aElm.outerHTML)) return;

	const rootNode = aElm.getRootNode() as Document;
	if (!aElm.hash || aElm.hash.length <= 1) {
		// aElm.hash may be empty when hash == "#"
		if (rootNode.location) rootNode.location.hash = "#";
		else rootNode.documentElement.scrollTop = 0;
	} else {
...
function ohpAfterSanitizeAttributes(node: Element) {
...
async function sanitizeAndApplyPatches(doc: Document): Promise<void> {
	doc.querySelectorAll("*").forEach((elm) => {
		let illSet = new Set<string>();
		for (let i = 0; i < elm.attributes.length; ++i) {
			let name = elm.attributes[i].name;
			if (name.indexOf("-") > 0) name = `${name.split("-")[0]}-*`;

			if (
				!BM_ALLOWED_ATTRS.contains(name) &&
				!illSet.has(elm.attributes[i].name)
			)
				illSet.add(elm.attributes[i].name);
		}
...
function applyUserInteractivePatches(doc: Document) {
	if (!doc.body.style) {
	...
	if (doc.body.style.overflow === "") doc.body.style.overflow = "auto";
	// avoid some HTML files unable to select text, only when 'user-select' is not set
	if (doc.body.style.userSelect === "") doc.body.style.userSelect = "text";
}

async function removeScriptTagsAndExtScripts(doc: Document): Promise<void> {
	...
	doc.querySelectorAll("link").forEach((node) => {
		...
		if (node.rel.contains("script")) node.parentNode!.removeChild(node);
		else if (
			node.rel.contains("preload") &&
			node.as &&
			node.as.contains("script")
		)
			node.parentNode!.removeChild(node);
	});
}

async function modifyAnchorTarget(doc: Document): Promise<void> {
	...
	doc.querySelectorAll("a").forEach((aElm) => {
		if (aElm.target === "_self") aElm.target = "_blank";
		...
		if (!regex.test(aElm.outerHTML)) {
			...
			if (!aElm.rel) {
				...
				if (!aElm.rel.includes("noopener")) aElm.rel += " noopener";
				if (!aElm.rel.includes("noreferrer")) aElm.rel += " noreferrer";
			}
		}
	});
}

async function restoreStateBySettings(
	doc: Document,
	settings: HtmlPluginSettings
): Promise<void> {
	// all[0] ==> <html>
	(doc.all[0] as HTMLElement).style.transformOrigin = "left top"; // CSS transform-origin
	(
		doc.all[0] as HTMLElement
	).style.transform = `scale(${settings.zoomValue})`;
...
function mapNativeHotkeys(app: App, cmdId: string): Hotkey[] {
	let ohks =
		app.hotkeyManager.getHotkeys(cmdId) ||
		app.hotkeyManager.getDefaultHotkeys(cmdId);

	const nhks: Hotkey[] = [];
	if (!ohks || ohks.length <= 0) return nhks;
	...
	for (let mod of modifiers) {
		switch (mod) {
			...
			case "Mod": // Mod = Cmd on MacOS/iOS/iPadOS and Ctrl on other OS
				if (isAppleSys ? !evt.metaKey : !evt.ctrlKey) return false;
				break;
		}
	}
	return true;
...
async function buildUserInteractiveFacilities(
	mainView: MainView
): Promise<void> {
	const searchBar: HTMLElement = mainView.searchBar;
	const iframe: HTMLIFrameElement = mainView.iframe;
	const settings: HtmlPluginSettings = mainView.settings;

	let isSearchBarVisible: boolean = false,
		hltAllNodes: boolean = false;
	let curText: string;
	let curIndex: number = -1;

	const allMatched: HTMLElement[][] = []; // array of array of mark element(s)
	const tmpMatched: HTMLElement[] = []; // array of temp. mark elements for combining later
	const mark = new Mark(iframe.contentWindow!.document.body);
	const addMatched = (node: HTMLElement) => {
		if (isUnselectableElement(node.parentElement!)) return;

		if (node.textContent && node.textContent.length != curText.length) {
			if (tmpMatched.length <= 0) {
				tmpMatched.push(node);
				return;
			}
			...
			for (let idx = 0; idx < tmpMatched.length; ++idx) {
				...
				if (tmpText2.length === curText.length) {
					...
					for (let i = 0; i <= idx; ++i) tmpMatched.shift();
					break;
				}
			}
		} else {
	...
	const clearObsMark = (array: HTMLElement[]) => {
	...
	const setObsMark = (array: HTMLElement[]) => {
		if (!array || array.length <= 0) return;
	...
	const clearAllMarks = (includeTags: boolean) => {
		if (includeTags) {
			mark.unmark(obsOpt);
			mark.unmark(tmpOpt);
			hltAllNodes = false;
	...
	const setAllMarks = (includeObs: boolean) => {
		if (allMatched.length <= 0) {
			if (includeObs) mark.mark(curText, obsOpt);
			else mark.mark(curText, tmpOpt);
		} else if (includeObs) {
			for (const elm of allMatched) {
				setObsMark(elm);
			}
		...
		if (allMatched.length > 0) hltAllNodes = true;
	};
	const toggleInputError = (showError: boolean) => {
		if (showError) {
			clearAllMarks(true);
	...
	const findNext = () => {
		if (hltAllNodes) clearAllMarks(false);

		// no matched
		if (!curText || allMatched.length <= 0) return;

		// unmark old node
		if (curIndex >= 0 && curIndex < allMatched.length)
			clearObsMark(allMatched[curIndex]);

		if (curIndex + 1 >= allMatched.length) curIndex = 0;
		else curIndex++;

		setObsMark(allMatched[curIndex]);
		allMatched[curIndex][0].scrollIntoView({
			behavior: "smooth",
			block: "center",
		});
	};
	const findPrev = () => {
		...
		if (curIndex - 1 < 0) curIndex = allMatched.length - 1;
		else curIndex--;
	...
	const findAll = (
		...
		tmpMatched.length = 0;
		curText = text;
		if (markAll) mark.mark(curText, obsOpt);
		else mark.mark(curText, tmpOpt);
		...
		if (allMatched.length <= 0) {
			...
			// update curIndex
			if (curIndex > allMatched.length) curIndex = allMatched.length - 1;
			else if (selObj && selObj.anchorNode) {
				// select nearest element as curIndex
				const sibNode =
					selObj.anchorNode.nextSibling ||
					selObj.anchorNode.parentElement;
				if (
					sibNode &&
					sibNode.nodeName === "SPAN" &&
					sibNode.nodeType === Node.ELEMENT_NODE &&
					(sibNode as Element).classList.contains(
						"ohp-temp-search-class"
					)
				) {
	...
	mainView.ZoomIn = () => {
		...
		(
			iframeDoc.all[0] as HTMLElement
		).style.transform = `scale(${settings.zoomValue})`;
		iframe.contentWindow!.focus();
	};
	mainView.ZoomOut = () => {
	...
	// add event handlers
	const input: HTMLInputElement = searchBar.querySelector("#ohpSearchInput")!;
	input.addEventListener("keyup", (evt) => {
		if (evt.altKey && evt.keyCode === 13) {
			// handle "select all" command when press Alt+Enter
			sall.click();
		} else if (evt.keyCode === 13) {
			// when press Enter key then perform search next
			next.click();
		}
	});
	const next: HTMLButtonElement = searchBar.querySelector("#ohpSearchNext")!;
	next.addEventListener("click", (evt) => {
		checkAndUpdateMatches();
		findNext();
	});
	const prev: HTMLButtonElement = searchBar.querySelector("#ohpSearchPrev")!;
	prev.addEventListener("click", (evt) => {
		checkAndUpdateMatches();
		findPrev();
	});
	const sall: HTMLButtonElement = searchBar.querySelector(
		"#ohpSearchSelectAll"
	)!;
	...
	const exit: HTMLSpanElement = searchBar.querySelector("#ohpSearchExit")!;
	...
	iframe.contentWindow!.addEventListener("keydown", (evt) => {
		if (evt.shiftKey && evt.key === "F3") {
			if (isSearchBarVisible) prev.click();
		} else if (evt.altKey && evt.key === "Enter") {
			if (isSearchBarVisible) prev.click();
		} else if (evt.altKey && evt.keyCode === 13) {
			// search select all Alt+Enter
			if (isSearchBarVisible) sall.click();
		} else if (evt.key === "F3") {
			// search next F3
			if (isSearchBarVisible) next.click();
		} else if (evt.key === "Escape") {
			// close search bar
			if (isSearchBarVisible) exit.click();
		} else {
			// ignore Mod keys
			switch (evt.key) {
				case "Control":
			...
			if (hksSearch && hksSearch.length > 0) {
				for (let hk of hksSearch) {
					if (
						checkHotkeyModifier(hk.modifiers, evt) &&
						ek === hk.key
					) {
						evt.preventDefault();
						mainView.openSearch();
	...
	if (iframe.contentDocument!.body.children.length > 0)
		iframe.contentDocument!.body.insertBefore(
			hlt_style,
			iframe.contentDocument!.body.children[0]
		);
	else iframe.contentDocument!.body.appendChild(hlt_style);

	if (!settings.zoomByWheelAndGesture) return;
	...
	iframe.contentWindow!.addEventListener(
		...
		(evt) => {
			...
			if (delta < 0) mainView.ZoomIn();
			else if (delta > 0) mainView.ZoomOut();

			if (py > cy) {
				iframe.contentWindow!.scroll({
					top: NP.minus(NP.times(origPy, settings.zoomValue), cy),
					behavior: "auto",
				});
			}
		},
	...
	iframe.contentWindow!.addEventListener(
		...
		(evt) => {
			...
			if (evt.touches.length === 2) {
				...
				pinchClientY = NP.divide(
					NP.plus(evt.touches[0].clientY, evt.touches[1].clientY),
					2
				);
				pinchPageY = NP.divide(
					NP.plus(evt.touches[0].pageY, evt.touches[1].pageY),
					2
				);
			}
		},
...
<div class="document-search-container" style="display: none; border: none; width: 100%" width="100%" id="ohpMainView">
  <div class="document-search">
    <input class="document-search-input" type="search" placeholder="${i18next.t(
		"editor.search.placeholder-find"
	)}" id="ohpSearchInput">
    <div class="document-search-buttons">
      <button class="document-search-button" aria-label="${
			isAppleSys ? "⇧F3" : "Shift + F3"
		}" aria-label-position="top" id="ohpSearchPrev">${i18next.t(
	"editor.search.label-previous"
)}</button>
      <button class="document-search-button" aria-label="F3" aria-label-position="top" id="ohpSearchNext">${i18next.t(
			"editor.search.label-next"
		)}</button>
      <button class="document-search-button" aria-label="${
			isAppleSys ? "⌥Enter" : "Alt + Enter"
		}" aria-label-position="top" id="ohpSearchSelectAll">${i18next.t(
	"editor.search.label-all"
)}</button>
	  <span class="document-search-close-button" aria-label="${i18next.t(
			"editor.search.label-exit-search"
		)}" aria-label-position="top" id="ohpSearchExit"></span>
    </div>
  </div>
</div>
...
const textModeConfig = {
	...
	CUSTOM_ELEMENT_HANDLING: {
		tagNameCheck: (tagName: string): boolean => Boolean(tagName), // allow all tags with custom element format "xxx-yyy"
...
const hrModeConfig = {
	...
	ALLOWED_URI_REGEXP:
		/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp|app):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,
